...
for vaccine_stage in vaccine_stage_result:
    ...
    for vaccine_type in vaccine_type_result:
        ...
        if vaccine_type_group_size_result[0]["sum"] != None:
            stage_total += int(vaccine_type_group_size_result[0]["sum"])
    ...
    stage_total_from_database = int(execute_sql_query(vaccine_group_size_query)[0]['sum'])
    if stage_total_from_database != stage_total:
        print(f'!!! math does not add up for "{vaccine_stage_name}", calculated total: {stage_total}, total from api: {stage_total_from_database} !!!')
    print(f'{vaccine_stage_name}: {stage_total}')
    for vaccine_type_total in vaccine_type_totals:
        print(f'\t{vaccine_type_total["type"]}: {vaccine_type_total["sum"]} ({round(int(vaccine_type_total["sum"]) * 100 / stage_total, 2)}%)')
    total += stage_total
print(f'total vaccinations: {total}')
#eligible_people_count_query = 'SELECT * from "83da473d-68e1-4120-93f4-817bafce88a2"'
#eligible_people_count_result = execute_sql_query(eligible_people_count_query)
#print(eligible_people_count_result)